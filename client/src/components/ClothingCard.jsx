...
import NormalizedClothingImage from "./NormalizedClothingImage";
export default function ClothingCard({ item, onEdit, onDelete, onFavorite, selectable, selected, onSelect }) {
  ...
  return <article className={`clothing-card ${selected ? "selected" : ""}`} onClick={() => selectable && onSelect(item._id)}>
    ...
    >{item.imageUrl ? <NormalizedClothingImage item={item} alt={item.name || item.category}/> : <span>{item.category}</span>}<span className="compatibility-count" title={`${item.compatibleWith?.length || 0} compatibilité${item.compatibleWith?.length === 1 ? "" : "s"}`}><Network size={13}/>{item.compatibleWith?.length || 0}</span></div>
