...
export default function Outfits() {
  ...
  const packingCandidates = openCapsule ? clothes.filter(candidate => !openCapsule.clothes.some(item => item._id === candidate._id)) : [];
  ...
  return <>
    ...
    {addingPackingItem && <Modal title="Ajouter à la liste bagages" onClose={() => setAddingPackingItem(false)}><p>Choisissez une pièce à emporter, même si elle ne fait partie d’aucune tenue.</p><div className="selector-grid replacement-grid">{packingCandidates.map(item => <ClothingCard key={item._id} item={item} selectable onSelect={addItemToPackingList}/>)}</div>{!packingCandidates.length && <p className="empty-filter-message">Toute la garde-robe est déjà dans cette capsule.</p>}</Modal>}
